...
> Priorizado em 17/09/2026 via matriz Impacto × Esforço, atualizado em 20/09/2026 (matriz v14).
...
- [CONCLUÍDO] Renomear cards "Reembolsos (Pagos)" → "Reembolsos Pagos" e "Pendentes de Reembolso" → "Reembolsos Pendentes" na aba Reembolsos (UX writing).
...
- [CONCLUÍDO] Documentação funcional (base de conhecimento do agente de suporte): documentar recursos, funções e fluxos e os porquês, em linguagem de usuário (como fazer, por que existe, o que acontece), a partir do que já foi construído (specs, PRDs, telas). NÃO subir as specs técnicas cruas: elas descrevem arquitetura, regras de segurança do banco e tokens. Sem dados reais (nomes, e-mails, tokens de clientes). Esforço baixo a médio com apoio de IA e revisão minha. Pré-requisito do item seguinte e insumo do futuro tutorial do RF-15.
- [CONCLUÍDO] Agente de suporte com Chatbase embutido no sistema: conta Pro do Chatbase disponível sem custo. O agente é treinado com a documentação funcional (item anterior) e embutido por link/código embed, explicando a ferramenta ao usuário antes de existir a página de tutoriais do RF-15. Cuidados: (1) escopo do widget: SOMENTE a área logada (componente dentro do AppShell); NÃO nas rotas públicas /p/<token> e /r/<token>, cujas URLs contêm tokens de acesso que um script de terceiros poderia ler; (2) não enviar identidade nem dados do usuário ao Chatbase; (3) instruções do agente: responder só com base na documentação, em português, e encaminhar para contato@triadetecnologiaesolucoes.com.br quando não souber; (4) conferir limites de mensagens do plano e a política de dados/LGPD do Chatbase (o histórico de conversas fica no serviço); (5) não há CSP no repositório; se existir regra de cabeçalhos na Cloudflare, liberar o domínio do Chatbase. Esforço de código mínimo (um componente com o snippet). Balão na área logada (Agenda, Pessoas & Equipe, Configurações, prancheta do show e Relatório de Produção); base em Chatbot/base-de-conhecimento/; evolução do agente descrita em Chatbot/oportunidades-chatbase-hub-manager-tour.md.
- [CONCLUÍDO] Relatório de Produção legível no celular: rolagem horizontal nas tabelas de Elenco & Logística, Documentos recebidos e Resumo de reembolsos (mesmo padrão do Rider Técnico); corrigiu também o balão de ajuda, que ficava escondido pela largura da página.
- Cores/contraste do Modo Palco
- Conferir no celular os filtros nativos do Modo Palco
- Corrigir sobreposição de texto no celular nas abas Reembolsos e Documentos do show (linha com flex-wrap; agravada com o A+). Passo de UX/UI curto antes do código.
- Preferência 'Assistente de ajuda: ligado/desligado': o Chatbase não oferece fechar o balão. Controle no grupo de botões da interface (A+, tema), dentro do menu de três linhas no celular; padrão ligado; ao desligar, salvar a escolha no navegador e recarregar sem carregar o widget; o controle continua visível para religar. Passo de UX/UI curto antes do código. Ao entregar, atualizar a base e o Q&A do agente.
...
- Recuperação de senha (fluxo 'Esqueci minha senha'): hoje a tela de login não tem recuperação. Escopo provável: link na tela de login, e-mail de redefinição pelo Supabase Auth, página de nova senha e mensagens em português. Depende do item do domínio oficial (URL de redirecionamento). Ao entregar, atualizar a base (arquivos 02 e 12) e o Q&A do agente.
...
- Revisão geral da interface mobile (passada final, depois dos demais). Incluir: avaliar layout empilhado do Elenco no Relatório de Produção no celular, para o status ('falta Hotel/Voucher...') aparecer sem rolar a tabela.
...
- Conversão automática de moeda estrangeira: ao detectar moeda 
  ...
  de cotação usar.
- Notificações por e-mail e push (produção ↔ casa de show): hoje as respostas da casa chegam à produção só dentro do sistema (aviso ao atualizar a prancheta). Desejado: e-mail quando a casa confirmar, sinalizar exceção ou responder, e aviso à casa quando a produção replicar; push depois. Planejar junto com o item 'Sininho de notificações não lidas'. Ao entregar, atualizar a base e o Q&A.
